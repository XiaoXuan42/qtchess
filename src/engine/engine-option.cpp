...
EngineOption::EngineOption(Type type, QString name,
                           QMap<Entry, QVariant> entries)
    : m_entries(entries), m_name(name), m_type(type) {}
...
EngineOption EngineOption::combobox(QString name, QString defaultValue,
                                    QStringList values) {
    return EngineOption(Type::ComboBox, name,
                        {{Default, defaultValue}, {Vars, values}});
}
...
EngineOption::Type EngineOption::type() const { return m_type; }

QString EngineOption::name() const { return m_name; }
