...
AbstractSettings::AbstractSettings(const QString& logicalGroup,
    ...
    if (subGroup != "") m_settings.beginGroup(subGroup);
}
...
QStringList AbstractSettings::keys() const { return m_settings.childKeys(); }
...
void AbstractSettings::unset(const QString& key) { m_settings.remove(key); }

void AbstractSettings::reset() {
    for (const QString& key : m_values.keys()) {
        if (m_settings.contains(key)) set(key, m_settings.value(key));
    }
}
