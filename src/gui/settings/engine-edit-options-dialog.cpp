...
EngineEditOptionsDialog::EngineEditOptionsDialog(EngineConfig config,
                                                 QWidget* parent)
    : QDialog(parent), ui(new Ui::EngineEditOptionsDialog), m_config(config) {
    ui->setupUi(this);
    QObject::connect(ui->ok, SIGNAL(clicked(bool)), this, SLOT(onOkClicked()));
    rebuildTable();
}

EngineEditOptionsDialog::~EngineEditOptionsDialog() { delete ui; }

void EngineEditOptionsDialog::onOkClicked() { emit accept(); }
...
void EngineEditOptionsDialog::rebuildTable() {
    ...
    for (const EngineOption& option : engine.options()) {
        ...
        switch (option.type()) {
            ...
            case EngineOption::SpinBox:
                ...
                break;
            default:
                qDebug() << "FIXME: option " << option.name()
                         << " does not have visual representation.";
                widget = new QWidget();
        }
    ...
    // Empty table.
    if (!table->item(0, 0)) return;

    // Resize table rows to just look good.
    const int height = 1.4 * QFontMetrics(table->item(0, 0)->font()).height();

    for (int i = 0; i < rowId; i++) table->setRowHeight(i, height);

    // And columns too
    table->resizeColumnToContents(0);
}

QLineEdit* EngineEditOptionsDialog::createLineEdit(QString key,
                                                   EngineOption option) {
    QString currentValue = option.value(EngineOption::Default).toString();

    if (m_config.options().contains(key)) currentValue = m_config.option(key);
...
QCheckBox* EngineEditOptionsDialog::createCheckBox(QString key,
                                                   EngineOption option) {
    ...
    QObject::connect(widget, &QCheckBox::clicked,
                     [this, key, widget](bool checked) {
                         m_config.setOption(key, checked ? "true" : "false");
                     });
    return widget;
}

QSpinBox* EngineEditOptionsDialog::createSpinBox(QString key,
                                                 EngineOption option) {
